  ...
  useEffect(() => {
    const getUserData = async () => {
      try {
        ...
        if (user.cardio && user.physical) {
          const cardio = user.cardio;
          const physical = user.physical;
          const exercise = cardio.concat(physical);
                  ...
                  <Link className='text-decoration-none' to={`/history/${exercise.type}/${exercise._id}`}>
                    {exercise.type === "cardio" ? (
                      ...
                      <div className="history-card physical-title d-flex">
                        <div className='d-flex align-items-center'><img alt="physical" src={physicalIcon} className="history-icon" /></div>
                        <div >
                          <p className='history-name'>{exercise.name}</p>
